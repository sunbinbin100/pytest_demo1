...
class RunDubbo(telnetlib.Telnet):
    ...
    def command(self, flag, str_=""):          # 模拟命令行输入
        data = self.read_until(flag.encode())  # 读取上一次命令执行后返回的内容（内容包括：elapsed: 33 ms.\r\ndubbo>）
        self.write(str_.encode() + b"\n")      # 输入新的命令(str_的值)并回车执行，下次执行本方法后，可读取本次执行的结果（本次连接未关闭时）
        return data
    ...
    def do(self, arg2):
        ...
        data = self.command(self.prompt, "本句话是下一个命令语句--但是是本方法的最后1个命令语句，每次运行都是重新连接，所以随便输入什么，不影响")
        self.write(b"exit\n")         # or：self.close()
